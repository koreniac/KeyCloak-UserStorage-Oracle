...
public class DemoUserStorageProvider implements UserStorageProvider,
	...
	public boolean updateCredential(RealmModel realm, UserModel userModel, CredentialInput input) {
		...
		String usermodel_name = userModel.getUsername();
		logger.info("updateCredential_getUsername == ("+usermodel_name+")");
	...
	public UserModel getUserById(String keycloakId, RealmModel realm) {
		...
		logger.info("StorageId.externalId = "+id);
		return new UserRepresentation(session, realm, model, userDAO.getUserById(id), userDAO);
		// return new UserRepresentation(session, realm, model, userDAO.getUserById(keycloakId), userDAO);
	}

	@Override
	public UserModel getUserByUsername(String username, RealmModel realm) {
		logger.info("getUserByUsername("+username+", "+realm+")");
	...
	public UserModel addUser(RealmModel realm, String username) {
		logger.info("addUser");
		User user = new User();
		logger.info("addUser_getUsername == ("+user.getUsername()+")");
	...
	public String getPassword(UserModel user) {
		// logger.info("getPassword("+user+")");
		String password = null;
		// String password = ((UserRepresentation)user).getPassword();
		if (user instanceof UserRepresentation) {
			logger.info("getPassword-UserRepresentation");
			password = ((UserRepresentation)user).getPassword();
		}else{
			logger.info("*** need to set no_cache at UserFederation Plugin-> CacheSetting-> CachePolicy-> NO_CACHE ***");
			//need to set no_cache at keycloak
		}
